...
|---|---|---|---|---|
| T-01 | `test_homepage_loads` | Startseite abrufbar | HTTP 200, HTML enthält «E-Fahrzeuge» | ✅ Bestanden |
| T-02 | `test_register_user` | Neuen Fahrgast registrieren | HTTP 200, Flash «Registrierung erfolgreich» | ✅ Bestanden |
| T-03 | `test_login_works` | Login mit korrekten Zugangsdaten | HTTP 200, Dashboard erscheint («Übersicht») | ✅ Bestanden |
| T-04 | `test_login_fails_with_wrong_password` | Login mit falschem Passwort | Flash «Ungültige Zugangsdaten», kein Redirect auf Dashboard | ✅ Bestanden |
| T-05 | `test_password_is_hashed_in_db` | Passwort-Hash prüfen | `password_hash` ≠ Klartext, beginnt mit `scrypt:` oder `pbkdf2:` | ✅ Bestanden |
| T-06 | `test_provider_can_create_scooter` | Anbieter legt neues Fahrzeug an | HTTP 200, Fahrzeug `SC-9999` in Datenbank vorhanden | ✅ Bestanden |
...
| T-08 | `test_battery_drains_after_rental` | Akkustand sinkt nach 5 km Fahrt um 10 % | `battery_level` = Startwert − 10 (min. 0) | ✅ Bestanden |
| T-09 | `test_rental_rejected_when_battery_too_low` | Ausleihe mit Fahrzeug < 10 % Akku | Kein Rental angelegt, Flash «Akkustand zu niedrig» | ✅ Bestanden |
| T-10 | `test_rental_rejected_with_wrong_unlock_code` | Ausleihe mit falschem Entriegelungscode | Kein Rental angelegt (`active count == 0`) | ✅ Bestanden |
| T-11 | `test_api_token_and_vehicle_list` | API-Token beziehen, Fahrzeugliste und Rentals abrufen | Token als JSON, Fahrzeug-/Rental-Liste als Array | ✅ Bestanden |
| T-12 | `test_api_rentals_requires_auth` | `GET /api/rentals` ohne Token | HTTP 401, `error: missing_or_invalid_token` | ✅ Bestanden |
...
| M-06 | Akkugrenze beim Beenden | Im Dashboard Endkilometer > verfügbarer Akku/2 eingeben | Flash-Fehlermeldung mit Akkudetails, Ausleihe bleibt aktiv | ✅ Bestanden |
